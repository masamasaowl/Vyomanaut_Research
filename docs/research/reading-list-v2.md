...
| **R-03** | Proof transfer on repair | `proof of retrievability erasure coded repair, remote data checking regenerating codes, audit continuity node replacement distributed storage.` | Auditability survives a shard moving to a replacement provider without the owner online — ADR-004's repair makes this not-quite-write-once | Pure dynamic-update work with no migration case |
