...
## Earnings Transparency and Trust (Paper 47)
...
Answer: Disclose the gross amount, the multiplier applied, and a plain-language reason category (e.g. "reliability score below threshold") for any release under 100%. Withhold the exact formula, score, and thresholds. Rendered through the same IC §14 copy contract used for error/status messaging, not a separate ad hoc earnings text system. The reason-category list itself is not yet finalised — that remains open alongside the scoring implementation. → [ADR-045](../decisions/ADR-045-earnings-transparency-disclosure.md), building on the [ADR-016](../decisions/ADR-016-payment-db-schema.md) addendum (gross amount + release multiplier columns) that supplies the underlying data.
