...
Open research and product questions live in a single canonical file:
[`docs/research/open-questions.md`](../research/open-questions.md). This section is a pointer
only — edit questions there, not here — so anyone searching this repository for open questions
finds one current answer instead of two copies that can quietly drift apart.

That file organizes questions into four groups:

- **Product Open Questions** — business decisions blocking private beta (5 open: OQ-001–OQ-005)
- **Engineering Open Questions — Telemetry** — answerable only by observing the live V2 system; none block the build (24 open, grouped by domain)
- **V3-Deferred Questions** — valid questions explicitly out of scope for V2 (16 rows, ~20 individual questions)
- **Additional Open Questions (Papers 42–49)** — later literature-review questions not yet triaged into the three tiers above (7 open, plus Q47-1 which has since closed)

All Tier 1 build-blocker questions are resolved. The closed record is summarized in §11.3 below,
with the full resolution history — including the recently-closed Q47-1 (ADR-045) — in
[`docs/research/answered-questions.md`](../research/answered-questions.md).
