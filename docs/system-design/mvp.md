...
### 9.1 `docs/decisions/ADR-031-demo-mode-network-profile.md` (new, to be created)

**Must contain:**
- Decision: single codebase, mode flag, NetworkProfile struct
- Supersedes the `--dev-mode` clause in ADR-029 for demo purposes
- The two complete profile definitions (ProductionProfile and DemoProfile) as the canonical
  reference
- The corrected `MinDistinctASNs=5` for demo (not 2 as in the pre-ADR analysis)
- The explicit list of parameters that must not vary between modes (wire formats, ShardSize,
  cryptographic primitives, row security policies)
- Cross-references to every ADR whose parameter values are overridden by the demo profile

**Must not change:** The logic of any existing ADR. ADR-031 overrides values, not decisions.

### 9.2 `docs/system-design/architecture.md`

**Must add:**
- A new subsection in §9 (Core Design Principles): "Profile-driven configuration — all
  mode-variable parameters live in `NetworkProfile`. Logic never branches on mode directly."
- A row in the Technology Stack table (§4): `Network Profile | internal/config.NetworkProfile | Single source of truth for all demo/prod parameter differences`
- A note in §8 (Minimum Viable Network) that thresholds come from `NetworkProfile.Min*`
  fields, with a pointer to ADR-031 for demo values
- A sentence in §24 (Deployment Topology) noting that demo topology runs all components on
  a single machine

**Must not change:** Any component description, trust boundary, security boundary, or ADR
reference. The architecture is the same in both modes.

### 9.3 `docs/system-design/requirements.md`

**Must add:**
- A new §5.10 (Mode Requirements): a short table mapping each `NetworkProfile` field to the
  FR or NFR it governs, noting that demo mode overrides the threshold value but not the
  requirement logic
- A note in §7.1 (Hard Constraints) that `ShardSize=262144` is a fixed constant in both
  modes — it is not in `NetworkProfile`
- A note on FR-053 (Readiness Gate) that threshold values come from the active
  `NetworkProfile`; the gate logic is unchanged between modes

**Must not change:** Any existing FR or NFR. The requirements are the same; only the
thresholds differ.

### 9.4 `docs/system-design/data-model.md`

**Must update:**
- §3 (Design Invariants): Invariant 5 note — "In demo mode with TotalShards=5, the constraint
  becomes `BETWEEN 0 AND 4`. The constraint value is emitted from `NetworkProfile.TotalShards-1`
  at migration generation time."
- `chunk_assignments` table definition: change the `shard_index` CHECK from the literal
  `BETWEEN 0 AND 55` to a comment: `-- generated from NetworkProfile.TotalShards-1; see
  migrations/generator.go`
- `repair_jobs` table definition: same change for `available_shard_count BETWEEN 16 AND 56`
- §7 (Materialised Views): add a note that `mv_provider_scores` interval literals are
  generated at microservice startup from `NetworkProfile.ScoreWindow{Short,Medium,Long}`;
  the view DDL in this document shows the production values for reference
- §9 (Migration Checklist): add two checklist items:
  - `[ ] Schema generated against the correct NetworkProfile (run migrations/generator.go with the target profile)`
  - `[ ] mv_provider_scores view regenerated at first startup of the target environment`

**Must not change:** Table schemas, row security policies, invariants 1–6. The data model
is mode-agnostic except for the two parameterised CHECK constraints.

### 9.5 `docs/system-design/interface-contracts.md`

**Must update:**
- §3.4 (Readiness Gate Contract): add a table row noting that all `"threshold"` values in the
  `GET /api/v1/admin/readiness` response body come from the active `NetworkProfile`; show both
  demo and production values for each condition
- §5.1 (`internal/crypto`): add a note that `DeriveMasterSecret` accepts `argon2Time`,
  `argon2Memory`, `argon2Threads` from `NetworkProfile`, not hardcoded constants
- §5.2 (`internal/erasure`): change the package comment to note that `DataShards`,
  `ParityShards`, `TotalShards` are injected from `NetworkProfile` via `NewEngine(profile)`
  rather than being package-level constants
- §5.3 (`internal/storage`): no change (ShardSize is a compile-time constant in both modes)
- §6 (PostgreSQL Row-Level Contracts): add a note under `chunk_assignments` that the
  `shard_index` CHECK constraint value is profile-dependent; reference §5.5 of this MVP doc
- §10 (Versioning Rules): add a clause: "NetworkProfile fields are not versioned separately
  from the codebase. A new field requires both a ProductionProfile value and a DemoProfile
  value in the same PR. The Go struct-literal syntax enforces this at compile time."

**Must not change:** Any protocol frame format, field offset, maximum size, 0-RTT policy,
Ed25519 signing convention, or error code. Wire contracts are identical in both modes.

### 9.6 `openapi.yaml`

**Must update:**
- The `GET /api/v1/admin/readiness` response schema: change all numeric threshold fields
  from hard-coded examples (e.g. `example: 56`) to add a description note:
  `description: "Threshold value from the active NetworkProfile; 56 in production, 5 in demo"`
- The `POST /api/v1/provider/register` request schema: add an optional `demo_asn` field
  (string, max 16 chars) used only when `VYOMANAUT_MODE=demo` to assign a synthetic ASN.
  Production registrations omit this field; the real ASN is looked up from the provider's
  IP address via a GeoIP/ASN database.

**Must not change:** Any request/response field types, HTTP methods, authentication
requirements, or error envelope structure. The API surface is identical in both modes.

### 9.7 Summary: what changes, what does not
