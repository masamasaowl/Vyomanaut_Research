...
### 14.2 File Availability Labels

`GET /api/v1/owner/files` (`internal/api/owner.go`) computes a per-file `Availability` field from the **worst** segment across the file (a multi-segment file is only as retrievable as its weakest segment; `available_shard_count`/`total_shard_count` are separately reported as sums across all segments and must not be used to infer availability themselves):

```go
okThreshold       := profile.DataShards + profile.LazyRepairR0
criticalThreshold := profile.DataShards   // the reconstruction floor
switch {
case worstSegmentAvailable >= okThreshold:       Availability = "OK"
case worstSegmentAvailable >= criticalThreshold: Availability = "DEGRADED"
default:                                          Availability = "CRITICAL"
}
```

`CRITICAL` specifically means *below* `DataShards` — the file cannot currently be reconstructed from available shards at all (not merely "very degraded"). This is exactly the condition `internal/repair`'s `PriorityEmergency` tier exists for ("at reconstruction floor; front of queue"), so `CRITICAL` files should always have an emergency repair job in flight or queued.

**Data-owner-facing label mapping** (resolves the vocabulary gap between this field and FR-019's "available / degraded / unavailable" wording — the underlying three-tier semantics already line up 1:1 once `CRITICAL`'s precise meaning is accounted for):

| API `Availability` | Data-owner label | Meaning shown to the owner |
| --- | --- | --- |
| `OK` | **Available** | File is healthy. |
| `DEGRADED` | **Degraded — repair in progress** | Some copies are missing; the file is still fully retrievable while repair catches up. |
| `CRITICAL` | **Temporarily unavailable — emergency repair in progress** | Below the minimum needed to retrieve right now; this file has emergency priority in the repair queue. |

`DEGRADED` must never be worded as if retrieval is at risk — it isn't; that is precisely the distinction `DEGRADED` vs `CRITICAL` exists to draw, and collapsing them into one label (e.g. showing both as "degraded") would either needlessly alarm owners at `DEGRADED` or dangerously undersell `CRITICAL`.

An estimated repair-completion time for `DEGRADED`/`CRITICAL` files is **not yet available from any data source** (no throughput/rate figure exists anywhere in `internal/repair` to convert queue position into a time estimate) — see ADR-037 for the resulting scope decision.
