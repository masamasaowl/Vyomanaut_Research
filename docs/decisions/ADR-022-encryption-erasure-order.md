...
Step 5 — AONT package
  ...
  (Three 16-byte word-widths — 48 bytes — larger than the original segment: one
   canary word (c_s) plus one 32-byte key-embedding block (c_{s+1}, which occupies
   two word-widths because K is 256 bits, not one — an earlier revision of this
   line said "one word larger," which undercounted the key block by a word.
   Confirmed against interface-contracts.md §5.1's AONTEncodeSegment post-condition
   and against AONTDecodePackage's own precondition, len(aontPackage) >= 64 =
   2×16 + 32.)
